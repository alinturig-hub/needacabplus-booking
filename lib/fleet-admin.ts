...
export async function fleetList(resource:FleetResource,request:Request){
 const url=new URL(request.url),pageSize=Math.min(100,Math.max(10,Number(url.searchParams.get('pageSize'))||20)),requestedPage=Math.max(1,Number(url.searchParams.get('page'))||1),search=(url.searchParams.get('search')||'').trim(),status=(url.searchParams.get('status')||'All').trim();
 ...
 const [items,summary]=await Promise.all([
  ...
  db.query<{total:string;active:string;suspended:string;last_synced:string|null}>(`SELECT COUNT(*)::text AS total,COUNT(*) FILTER (WHERE suspended=false)::text AS active,COUNT(*) FILTER (WHERE suspended=true)::text AS suspended,MAX(synced_at)::text AS last_synced FROM ${table}`)
 ]);
 const totals=summary.rows[0]||{total:'0',active:'0',suspended:'0',last_synced:null};
 return {items:items.rows,total,page,pageSize,summary:{total:Number(totals.total),active:Number(totals.active),suspended:Number(totals.suspended),lastSynced:totals.last_synced}};
}
