...
export async function fleetList(resource:FleetResource,request:Request){
 ...
 const values:unknown[]=[],where:string[]=["(lower(trim(company)) LIKE 'taxi services (plymouth) ltd%' OR lower(trim(company)) LIKE 'plymouth taxi%')"];
 ...
 const [items,summary,companies]=await Promise.all([
  ...
  db.query<{total:string;active:string;suspended:string;last_synced:string|null}>(`SELECT COUNT(*)::text AS total,COUNT(*) FILTER (WHERE suspended=false)::text AS active,COUNT(*) FILTER (WHERE suspended=true)::text AS suspended,MAX(synced_at)::text AS last_synced FROM ${table} WHERE (lower(trim(company)) LIKE 'taxi services (plymouth) ltd%' OR lower(trim(company)) LIKE 'plymouth taxi%')`),
  db.query<{company:string}>(`SELECT DISTINCT company FROM ${table} WHERE company IS NOT NULL AND (lower(trim(company)) LIKE 'taxi services (plymouth) ltd%' OR lower(trim(company)) LIKE 'plymouth taxi%') ORDER BY company`)
 ]);
