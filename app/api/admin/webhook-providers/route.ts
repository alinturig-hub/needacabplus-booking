...
const providerFields=z.object({
 ...
 apiKeyHeader:z.string().trim().max(100).default('x-api-key'),apiKey:z.string().max(2000).default(''),enabled:z.boolean().default(true)
});
const webhookFields=z.object({
 ...
 eventUrlSuffix:z.string().trim().min(1).max(300).refine(value=>value.startsWith('/')&&!value.startsWith('//'),'URL suffix must start with /'),
...
export async function GET(){
 ...
 try{const db=database();const [providers,webhooks]=await Promise.all([
  db.query('SELECT id,name,description,base_url,api_key_header,enabled,created_at,updated_at FROM webhook_providers ORDER BY name'),
...
export async function POST(request:Request){
 ...
 try{const body=createSchema.parse(await request.json());const id=randomUUID();const db=database();
  if(body.type==='provider')await db.query('INSERT INTO webhook_providers (id,name,description,base_url,api_key_header,api_key_encrypted,enabled) VALUES ($1,$2,$3,$4,$5,$6,$7)',[id,body.name,body.description,body.baseUrl.replace(/\/$/,''),body.apiKeyHeader,body.apiKey?encryptCredentials({token:body.apiKey}):'',body.enabled]);
