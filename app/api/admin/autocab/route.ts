...
const connectionUpdateSchema=z.object({type:z.literal('connection'),id:z.string().uuid(),name:z.string().trim().min(2).max(80),baseUrl:z.string().url().max(500).refine(value=>new URL(value).protocol==='https:','Use an HTTPS URL'),authType:z.enum(['none','api_key','bearer','basic']),apiKeyHeader:z.string().trim().min(1).max(100),token:z.string().max(2000).optional().default(''),username:z.string().max(250).optional().default(''),password:z.string().max(1000).optional().default('')}).strict();
const endpointUpdateSchema=endpointSchema.omit({type:true}).extend({type:z.literal('endpoint'),id:z.string().uuid()});
...
export async function PUT(request:Request){
 ...
 try{
  const db=database();
  if((raw as {type?:unknown})?.type==='connection'){
   const body=connectionUpdateSchema.parse(raw),current=await db.query<{auth_type:string;credentials_encrypted:string}>('SELECT auth_type,credentials_encrypted FROM api_connections WHERE id=$1 AND provider=$2',[body.id,'autocab']);
   if(!current.rowCount)return Response.json({error:'Autocab connection not found.'},{status:404});
   let encrypted=current.rows[0].credentials_encrypted;
   const supplied=body.authType==='basic'?Boolean(body.username&&body.password):body.authType==='none'||Boolean(body.token);
   if(body.authType!==current.rows[0].auth_type&&!supplied)return Response.json({error:'Enter credentials when changing the authentication type.'},{status:400});
   if(supplied){const credentials:Record<string,string>=body.authType==='basic'?{username:body.username,password:body.password}:body.authType==='none'?{}:{token:body.token};encrypted=encryptCredentials(credentials)}
   await db.query('UPDATE api_connections SET name=$1,base_url=$2,auth_type=$3,api_key_header=$4,credentials_encrypted=$5,updated_at=now() WHERE id=$6',[body.name,body.baseUrl.replace(/\/$/,''),body.authType,body.apiKeyHeader,encrypted,body.id]);
   return Response.json({ok:true});
  }
  const body=endpointUpdateSchema.parse(raw),owner=await db.query('SELECT id FROM api_connections WHERE id=$1 AND provider=$2',[body.connectionId,'autocab']);
  if(!owner.rowCount)return Response.json({error:'Autocab connection not found.'},{status:404});
  const result=await db.query('UPDATE api_endpoints SET connection_id=$1,name=$2,action_key=$3,method=$4,path=$5,description=$6,request_example=$7::jsonb,updated_at=now() WHERE id=$8',[body.connectionId,body.name,body.actionKey,body.method,body.path,body.description,JSON.stringify(JSON.parse(body.requestExample)),body.id]);
  if(!result.rowCount)return Response.json({error:'API endpoint not found.'},{status:404});return Response.json({ok:true});
 }catch(error){
  if(error instanceof z.ZodError)return Response.json({error:error.issues[0]?.message||'Check the API details.'},{status:400});
  if(error instanceof Error&&error.message.includes('duplicate key'))return Response.json({error:'That connection name or action key already exists.'},{status:409});
  return unavailable(error);
