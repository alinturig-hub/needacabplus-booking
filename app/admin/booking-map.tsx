...
export default function BookingMap({pickup,destination}:{pickup:Point;destination:Point}){
 ...
 useEffect(()=>{
  ...
  map.once('load',async()=>{
   ...
   try{const response=await fetch(`https://router.project-osrm.org/route/v1/driving/${pickupLongitude},${pickupLatitude};${destinationLongitude},${destinationLatitude}?overview=full&geometries=geojson`,{signal:abort.signal});if(response.ok){const result=await response.json() as {routes?:{geometry?:{coordinates?:number[][]}}[]};coordinates=result.routes?.[0]?.geometry?.coordinates||coordinates}}catch{}
   if(abort.signal.aborted)return;
   map.addSource('route',{type:'geojson',data:{type:'Feature',properties:{},geometry:{type:'LineString',coordinates}}});
   map.addLayer({id:'route-outline',type:'line',source:'route',paint:{'line-color':'#101312','line-width':8,'line-opacity':.72}});
   map.addLayer({id:'route',type:'line',source:'route',paint:{'line-color':'#f2dd4a','line-width':4}});
  });
  return()=>{abort.abort();markers.forEach(marker=>marker.remove());map.remove()};
